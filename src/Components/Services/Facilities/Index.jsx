function Facilities() {
  return (
    <div className="relative lg:h-screen">
      <img
        className="absolute top-0 left-0 w-full h-full object-cover"
        src="https://media-public.canva.com/McoUM/MAEPSeMcoUM/1/s2.jpg"
        alt="Background"
      />
      <div className="relative p-5 w-full">
        <div className="">
        <section className="mt-10  px-8">
          <h2 className="text-2xl lg:text-5xl font-semibold text-center mb-5 text-white">
            OUR BEST <span className="text-[rgb(243,157,36)]">SERVICES</span>
          </h2>
          <div className="w-40 border-[rgb(243,157,36)] m-auto"></div>
          <p className="text-white text-start mt-5 lg:text-2xl lg:w-1/2 m-auto">
            We always do our best in serving our customers and give them a satisfied impression.
          </p>
        </section>
        <div className="flex h-auto flex-col justify-between gap-20 lg:gap-10 mt-10 px-5 lg:px-0">
          <div className="m-auto lg:w-[60%]  flex lg:flex-row justify-around gap-10">
            <div className="flex flex-col gap-2 items-center text-center">
              <img
                className="rounded-full h-20 w-20 "
                src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcR4j3V68F8TRov0rAQqFHX3ZKi1DyQcmWk-Aw&s"
                alt="Marketing"
              />
              <p className="text-[rgb(243,157,36)] text-xl lg:text-2xl font-semibold">FRONT-END</p>
            </div>
            <div className="flex flex-col gap-2 items-center text-center">
              <img
                className="rounded-full h-20 w-20 h-10"
                src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQIWY9y29tu4KUGNmrdB2b-IHKFSB3uPzOuKQ&s"
                alt="Relation"
              />
              <p className="text-[rgb(243,157,36)] text-xl lg:text-2xl font-semibold">BACKEND</p>
            </div>
          </div>
          <div className="m-auto lg:w-[60%] lg:flex-row flex gap-10 justify-around ">
            <div className="flex flex-col gap-2 items-center text-center">
              <img
                className="rounded-full h-20 w-20 h-10"
                src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTlBsvmlvo3BsLoYPoD_WcLeKW6h0aZPsSKfg&s"
                alt="Strategy"
              />
              <p className="text-[rgb(243,157,36)] text-xl lg:text-2xl font-semibold">DSA</p>
            </div>
            <div className="flex flex-col gap-2 items-center text-center">
              <img
                className="rounded-full h-20 w-20 h-10 w-10"
                src="data:image/png;base64,"
                alt="Target"
              />
              <p className="text-[rgb(243,157,36)] text-xl lg:text-2xl font-semibold">GITHUB REPO</p>
            </div>
          </div>
        </div>
      </div>
      </div>
    </div>
  );
}
export default Facilities;